...
function example2() {
  ...
  const data1 = convertTextToTrainingData(text, 3, 1);
  ...
  console.log('\n--- パターン2: ウィンドウ5、ストライド2 ---');
  const data2 = convertTextToTrainingData(text, 5, 2);
  ...
  console.log('\n--- パターン3: ウィンドウ7、ストライド3 ---');
  const data3 = convertTextToTrainingData(text, 7, 3);
...
function example3() {
  ...
  const trainingData = convertMultipleTextsToTrainingData(texts, 4, 2);
...
async function example4() {
  ...
  const slidingWindowData = convertTextToTrainingData(longText, 5, 1);

  console.log(`生成されたトレーニングデータ数: ${slidingWindowData.length}`);

  // 語彙を作成
  const vocab = createVocab(slidingWindowData);
  console.log(`\n語彙サイズ: ${vocab.length}`);

  // モデルを作成
  const llm = new SimpleLLM(vocab, 16, 2);
